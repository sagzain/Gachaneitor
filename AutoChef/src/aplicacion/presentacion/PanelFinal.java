    ...
    public PanelFinal() {
        ...
        lblNewLabel_1.addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                    retornar = true;
                    for(Component component : ((Container)getParent().getParent().getComponent(0)).getComponents())
                    {
                       if(((ReutilizablePanelBoton)component).getLabel().equals("Siguiente"))
                       {
                          ((ReutilizablePanelBoton)component).setRegresar(true);
                          
                          for(MouseListener listener : ((ReutilizablePanelBoton)component).getMouseListeners())
                          {
        ...
        lblNewLabel_2.addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                ...
                for(Component component : ((Container)getParent().getParent().getComponent(0)).getComponents())
                {
                   if(((ReutilizablePanelBoton)component).getLabel().equals("Siguiente"))
                   {
                      ((ReutilizablePanelBoton)component).setRegresar(false);
                      for(MouseListener listener : ((ReutilizablePanelBoton)component).getMouseListeners())
                      {
                          listener.mouseClicked(e);
                      }
